    ...
    const load = async () => {
      try {
        const [s, c, a, b] = await Promise.all([
          studentsAPI.get(id), studentsAPI.classes(id),
          studentsAPI.attendance(id), studentsAPI.billing(id),
        ]);
        setStudent(s.data); setClasses(c.data); setAttendance(a.data); setBilling(b.data);
    ...
    <div>
      ...
      <Tabs defaultValue="info" data-testid="student-detail-tabs">
        <TabsList className="mb-4">
          ...
          <TabsTrigger value="clases"><BookOpen className="h-4 w-4 mr-1" /> Clases</TabsTrigger>
        ...
        <TabsContent value="clases">
          {classes.length === 0 ? <EmptyState icon={BookOpen} title="Sin clases" description="El alumno no est\u00e1 inscripto en ninguna clase." /> : (
            <Card className="shadow-sm"><Table><TableHeader><TableRow><TableHead>Clase</TableHead><TableHead>Tipo</TableHead><TableHead>Profesor</TableHead><TableHead>Fecha inscripci\u00f3n</TableHead></TableRow></TableHeader>
              <TableBody>{classes.map(c => (
                <TableRow key={c.id}><TableCell className="font-medium">{c.class_name}</TableCell><TableCell>{c.class_type_name || '-'}</TableCell><TableCell>{c.teacher_name || '-'}</TableCell><TableCell className="font-mono text-sm">{c.enrollment_date}</TableCell></TableRow>
        ...
        <TabsContent value="asistencia">
          {attendance.length === 0 ? <EmptyState icon={ClipboardCheck} title="Sin registros" description="No hay registros de asistencia." /> : (
            <Card className="shadow-sm"><Table><TableHeader><TableRow><TableHead>Fecha</TableHead><TableHead>Clase</TableHead><TableHead>Estado</TableHead><TableHead>Notas</TableHead></TableRow></TableHeader>
              <TableBody>{attendance.map(a => (
                <TableRow key={a.id}>
                  <TableCell className="font-mono text-sm">{a.schedule_date} {a.start_time}-{a.end_time}</TableCell>
